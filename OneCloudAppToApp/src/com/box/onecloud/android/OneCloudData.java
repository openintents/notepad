...
public class OneCloudData implements Parcelable {
     ...
     * Default constructor.
     *
     * @param binder OneCloudInterface.
     */
     ...
     * @param token   OneCloud token. This is not arbitrary. You must have obtained this by getting the token of a OneCloudData object you had in the past.
     * @return OneCloudData object or null of one could not be retrieved.
     * @throws NoSuchMethodException Thrown if the Box app installed does not yet support this method (you should fail gracefully and/or ask the user to upgrade their Box app).
     */
     ...
     *
     * @param listener An upload listener which you can use to monitor the upload progress. This can be null if you do not need to monitor the upload.
     * @throws RemoteException Thrown if there was a problem triggering the upload. Note, this does not get thrown if there was an error *during* and upload.
     */
    public void uploadNewVersion(final UploadListener listener) throws RemoteException {
        if (!isBinderValid()) {
            return;
        }
        mBinder.uploadNewVersion(
                new FileUploadCallbacks.Stub() {
     ...
     *
     * @param newFileName The new file name that the file will take on in Box.
     * @param listener    An upload listener which you can use to monitor the upload progress. This can be null if you do not need to monitor the upload.
     * @throws RemoteException Thrown if there was a problem triggering the upload. Note, this does not get thrown if there was an error *during* and upload.
     */
    public void uploadNewVersion(final String newFileName, final UploadListener listener) throws RemoteException {
        ...
        mBinder.uploadNewVersionWithNewName(
                newFileName, new FileUploadCallbacks.Stub() {
     ...
     *
     * @param suggestedFileName The file name that will be suggested to the user for the new file.
     * @param listener          An upload listener which you can use to monitor the upload progress. This can be null if you do not need to monitor the upload.
     * @throws RemoteException Thrown if there was a problem triggering the upload. Note, this does not get thrown if there was an error *during* and upload.
     */
    public void uploadNewFile(final String suggestedFileName, final UploadListener listener) throws RemoteException {
        ...
        mBinder.uploadNewFile(
                suggestedFileName, new FileUploadCallbacks.Stub() {
     ...
     * Laucnh the Box app.
     *
     * @throws RemoteException Thrown if the connection to Box is no longer active.
     */
    ...
    public interface UploadListener {
         ...
         * @param totalBytes       The total bytes that will have transferred when the upload has completed.
         */
